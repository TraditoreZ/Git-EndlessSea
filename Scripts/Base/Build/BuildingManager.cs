...
public class BuildingManager : NetworkBehaviour
{
    ...
    public CubeDirectionType GetTargerCubeDirection()
    {
        ...
        else if (cubeHit.normal.normalized + cubeHit.collider.transform.forward == Vector3.zero)
            return CubeDirectionType.Back;
        else if (cubeHit.normal.normalized - cubeHit.collider.transform.right == Vector3.zero)
            return CubeDirectionType.Right;
        else if (cubeHit.normal.normalized + cubeHit.collider.transform.right == Vector3.zero)
            return CubeDirectionType.Left;
        else if (cubeHit.normal.normalized - cubeHit.collider.transform.up == Vector3.zero)
    ...
    public CubeDirectionType GetSelfCubeDirection()
    {
        ...
        if (cubeHit.normal.normalized - BuildAidSystem.instance.prefabTrans.forward == Vector3.zero)
            return CubeDirectionType.Back;
        else if (cubeHit.normal.normalized + BuildAidSystem.instance.prefabTrans.forward == Vector3.zero)
            return CubeDirectionType.Forward;
        else if (cubeHit.normal.normalized - BuildAidSystem.instance.prefabTrans.right == Vector3.zero)
            return CubeDirectionType.Left;
        else if (cubeHit.normal.normalized + BuildAidSystem.instance.prefabTrans.right == Vector3.zero)
            return CubeDirectionType.Right;
        else if (cubeHit.normal.normalized - BuildAidSystem.instance.prefabTrans.up == Vector3.zero)
            return CubeDirectionType.Down;
        else if (cubeHit.normal.normalized + BuildAidSystem.instance.prefabTrans.up == Vector3.zero)
            return CubeDirectionType.Up;
        return CubeDirectionType.Down;
    ...
    public static ItemsTpye SeleteItemsType(ItemType type)
    {
        ...
        else if ((int)type > 1000 && (int)type < 2000)
