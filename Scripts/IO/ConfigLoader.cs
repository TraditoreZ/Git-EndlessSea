...
public static class ConfigLoader
{
    ...
    /// 加载配置文件，并形成字典结构
    /// </summary>
    /// <param name="ConfigFile">配置文件名，配置文件必须放在StreamingAssets文件中</param>
    /// <returns>构建好的字典，字典中主键是状态类名，子键是转换条件Id，子值是输出状态Id</returns>
    public static Dictionary<string, Dictionary<string, string>> Load(string ConfigFile)
    {
        if (Cache.ContainsKey(ConfigFile))
            return BuildDic(Cache[ConfigFile]);
        
        sourcePath = Application.streamingAssetsPath + @"/" + ConfigFile;
